...
export class ResumeService {
  ...
  getSkillRelations(skillId: string) {
    return this.http.get<WorkerSkillRelationDomain>(`${this.url}/tech-skills/related/${skillId}/${this.workerIdMe}`) ?? {};
  }
  ...
  getEmploymentHistory(workHistory:WorkHistoryDomain|undefined):ProjectParent[] {
    ...
    return workHistory.employmentHistory.map(value => {
      return {
        ...
        endDate: value.employmentEnd ? new Date(value.employmentEnd) : null,
  ...
  projectDomainToProject(projectDomain: ProjectDomain): Project {
    return {
      endDate: projectDomain.projectEnd ? new Date(projectDomain.projectEnd) : null,
